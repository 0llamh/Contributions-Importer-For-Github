    ...
    for c in self.get_all_commits(last_committed_date + 1):
      ...
      for broken_stats in stats.iterate_insertions(self.commit_max_changes):
        ...
        if time.strftime("%Y-%m-%d", time.localtime(last_committed_date)
          ...
          if commits_for_last_day > random(
          ) * (self.max_commits_per_day[1] - self.max_commits_per_day[0]) + self.max_commits_per_day[0]:
            print('    Commit skipped because the maximum amount of commit for ' +
                  time.strftime("%Y-%m-%d", time.localtime(last_committed_date)) + ' exceeded')
            continue
        else:
          commits_for_last_day = 1
        print('    Commit at: ' +
      ...
      for b in repo.branches:
        for c in repo.iter_commits(b.name):
          if c.committed_date < ignore_before_date or (
            self.ignore_before_date is not None and c.committed_date < self.ignore_before_date):
            continue
